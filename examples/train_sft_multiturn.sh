...
nproc_per_node=4
CONFIG_PATH=src/verl/config/SFT

PROJECT_NAME=
EXP_NAME=sft-qwen3-4b

TRAIN_FILES=sft_multiturn_train.parquet
VAL_FILES=sft_multiturn_test.parquet
...
torchrun --standalone --nnodes=1 --nproc_per_node=$nproc_per_node --master_port=123456 \
    ...
    model.partial_pretrain=/models/qwen3-4b \
    optim.lr=5e-6 \
    ...
    2>&1 | tee -a log/${PROJECT_NAME}_${EXP_NAME}.log
